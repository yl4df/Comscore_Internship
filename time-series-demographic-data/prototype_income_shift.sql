...
DROP TABLE IF EXISTS yunlu_proj2_income;
CREATE TABLE yunlu_proj2_income
( web_id bigint,
  month_id int,
  hh_income_id bigint,
  UV numeric(20,6),
...
SET @pop_id = 840;
SET @web_list = 0;
SET @tablename= 'yunlu_proj2_income';
...
WHILE @month_id > @enddate
   BEGIN
    ...
    from @ltt_table where location_id = @loc_id and population_id = @pop_id and web_id in (select web_id from yunlu_top100)
    group by web_id, month_id,hh_income_id;
...
--append web_name
DROP TABLE IF EXISTS yunlu_proj2_income_name_noncat;
CREATE TABLE yunlu_proj2_income_name_noncat AS
select h.web_name, i.* from yunlu_proj2_income as i
left join comScore.mm200_hierarchy_web_lookup_232m as h
on i.web_id = h.web_id
DISTRIBUTED RANDOMLY;

--append category name
DROP TABLE IF EXISTS yunlu_proj2_income_name_cat;
CREATE TABLE yunlu_proj2_income_name_cat AS
select b.cat_subcat_name as web_name, a .web_id, a.month_id, a.hh_income_id, a.uv, a.page_views, a.duration from (select * from yunlu_proj2_income_name_noncat where web_name isnull) as a
inner join Comscore.mm200_cat_subcat_lookup as b
on a.web_id = b.cat_subcat_id
DISTRIBUTED RANDOMLY;

--union two tables above
DROP TABLE IF EXISTS yunlu_proj2_income_name;
CREATE TABLE yunlu_proj2_income_name AS
select * from yunlu_proj2_income_name_noncat where web_name is not null
union
select * from yunlu_proj2_income_name_cat
DISTRIBUTED RANDOMLY;
...
DROP TABLE IF EXISTS yunlu_proj2_income_filtered;
CREATE TABLE yunlu_proj2_income_filtered AS
SELECT *
from yunlu_proj2_income_name where web_id in (select web_id from yunlu_proj2_income_name group by web_id, hh_income_id having count(*)=29) 
DISTRIBUTED RANDOMLY;


--append descriptive text
DROP TABLE IF EXISTS yunlu_proj2_income_text;
CREATE TABLE yunlu_proj2_income_text AS
SELECT a.web_name, a.web_id, a.month_id, a.hh_income_id, b.value_label, a.uv, a.page_views, a.duration 
from yunlu_proj2_income_filtered as a
inner join (
select * from comScore.mm200_demographics_lookup where demo_name = 'hh_income_id' and demo_value in ( 84003, 84004, 84005, 84006, 84007, 84011)
) as b
on a.hh_income_id = b.demo_value
DISTRIBUTED RANDOMLY;


--append category_name
DROP TABLE IF EXISTS yunlu_proj2_income_category;
CREATE TABLE yunlu_proj2_income_category AS
select a.*, b.category_id, c.cat_subcat_name from yunlu_proj2_income_text as a
...
Update yunlu_proj2_income_category
...
DROP EXTERNAL TABLE IF EXISTS yunlu_proj2_income_result;
CREATE WRITABLE EXTERNAL TABLE yunlu_proj2_income_result
(
LIKE yunlu_proj2_income_category
)
LOCATION
(
'gpfdist://csia2gpl06-1:8081/yuli/DATA/yunlu_income_prototype.txt'
)
FORMAT 'text' (delimiter E'\t' null 'null' escape 'off')
ENCODING 'UTF8';

INSERT INTO yunlu_proj2_income_result
SELECT *
from yunlu_proj2_income_category;
